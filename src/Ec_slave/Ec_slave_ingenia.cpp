...
void Ec_slave_ingenia::transfer_tx_pdo()
{
    ...
    uint8_t Operation_mode = EC_READ_U8(domain_i_pd + off_tx_pdo_4);

    std::cout << "INGENIA_TXPDO: " << slave_ns << " | " << Status_Word << ", " << Actual_position << ", " << Actual_velocity << ", " << uint16_t(Operation_mode) << std::endl;
}
